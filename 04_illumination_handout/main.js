...
var enableusercontrolledcamera = false;   
// when set to true UserControlledCamera class in framework.js is utilized (specifically designed for controlling camera position and orientation based on keyboard and mouse inputs)
// when set to false initInteraction function is used in main.js  (This is more generic currently adapted for camera rotation but also can be adapted to a wide range of applications beyond just camera control for instance performing interactive simulations )

if(enableusercontrolledcamera){
...
function init(resources) {
  ...
  if(enableusercontrolledcamera){
    //setup camera
    cameraStartPos = vec3.fromValues(0, -0.8, -20);
